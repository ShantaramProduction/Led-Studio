...
type InspectorTab = 'screen' | 'cabinet' | 'calc' | 'canvas' | 'project';
...
const translations = {
  ru: {
    ...
    open: 'Открыть', duplicate: 'Дублировать', delete: 'Удалить', archive: 'Архив', export: 'Экспорт', updated: 'Изменён', created: 'Создан', author: 'Автор', version: 'Версия', description: 'Описание',
    newScreen: '+ Новый экран', numbering: 'Нумерация', map: 'Карта', library: 'Библиотека', settings: 'Настройки', fit: 'Вписать', project: 'Проект', screens: 'Экраны', screen: 'Экран', cabinet: 'Кабинет', calc: 'Расчет', canvas: 'Холст', addScreen: '+ Добавить экран',
    ...
    fitView: 'Вписать вид', contextAddScreen: 'Добавить экран здесь', snapToGrid: 'Привязка к сетке', rulers: 'Линейки', guides: 'Направляющие', layers: 'Слои экранов', visible: 'Видимый', locked: 'Заблокирован', lock: 'Замок', eye: 'Вид',
    panHint: 'Средняя кнопка мыши или Alt+ЛКМ двигает холст. Колесо мыши зумит в курсор. Shift при перетаскивании экрана ограничивает движение по оси.', logout: 'Выйти', projectInfo: 'Информация проекта', actions: 'Действия',
  },
  en: {
    ...
    open: 'Open', duplicate: 'Duplicate', delete: 'Delete', archive: 'Archive', export: 'Export', updated: 'Updated', created: 'Created', author: 'Author', version: 'Version', description: 'Description',
    newScreen: '+ New Screen', numbering: 'Numbering', map: 'Map', library: 'Library', settings: 'Settings', fit: 'Fit', project: 'Project', screens: 'Screens', screen: 'Screen', cabinet: 'Cabinet', calc: 'Calc', canvas: 'Canvas', addScreen: '+ Add screen',
    ...
    fitView: 'Fit view', contextAddScreen: 'Add screen here', snapToGrid: 'Snap to grid', rulers: 'Rulers', guides: 'Guides', layers: 'Screen layers', visible: 'Visible', locked: 'Locked', lock: 'Lock', eye: 'Eye',
    panHint: 'Middle mouse or Alt+Left Mouse pans the canvas. Mouse wheel zooms into cursor. Shift while dragging locks movement to an axis.', logout: 'Logout', projectInfo: 'Project info', actions: 'Actions',
  },
};

type T = typeof translations.ru;

const normalizeProject = (raw: Partial<ProjectState>, language: Language): ProjectState => {
  const now = new Date().toISOString();
  const screens = (raw.screens?.length ? raw.screens : makeInitialScreens(language)).map((screen) => ({ visible: true, locked: false, ...screen }));
  return {
    id: raw.id ?? `project-${Date.now()}`,
    projectName: raw.projectName ?? (language === 'ru' ? 'LED-проект' : 'LED Project'),
    description: raw.description ?? '',
    author: raw.author ?? 'admin',
    version: raw.version ?? 1,
    screens,
    activeScreenId: raw.activeScreenId ?? screens[0].id,
    numberingMode: raw.numberingMode ?? 'rows',
...
const summarizeProject = (project: ProjectState): ProjectCard => {
  const totals = project.screens.reduce((acc, draft) => {
    const model = calculateScreen({ name: draft.name, cabinet: getCabinet(draft.cabinetId), columns: toNumber(draft.columns, 1), rows: toNumber(draft.rows, 1) });
    return {
      cabinets: acc.cabinets + model.calculated.cabinets,
      areaM2: acc.areaM2 + model.calculated.areaM2,
      weightKg: acc.weightKg + model.calculated.weightKg,
      avgPowerKw: acc.avgPowerKw + model.calculated.avgPowerKw,
      maxPowerKw: acc.maxPowerKw + model.calculated.maxPowerKw,
    };
  }, { cabinets: 0, areaM2: 0, weightKg: 0, avgPowerKw: 0, maxPowerKw: 0 });
  return { id: project.id, name: project.projectName, description: project.description, screens: project.screens.length, version: project.version, updatedAt: project.updatedAt, ...totals };
};
...
export default function Home() {
  ...
  const screen = calculateScreen({ name: activeDraft.name, cabinet, columns: toNumber(activeDraft.columns, 1), rows: toNumber(activeDraft.rows, 1) });

  const projectTotals = useMemo(() => screens.reduce((totals, draft) => {
    const model = calculateScreen({ name: draft.name, cabinet: getCabinet(draft.cabinetId), columns: toNumber(draft.columns, 1), rows: toNumber(draft.rows, 1) });
    return {
  ...
  }, { cabinets: 0, areaM2: 0, weightKg: 0, avgPowerKw: 0, maxPowerKw: 0 }), [screens]);
  ...
  const cabinets = Array.from({ length: screen.calculated.cabinets }, (_, index) => {
    ...
    const snakeNumber = row % 2 === 0 ? row * screen.columns + col + 1 : row * screen.columns + (screen.columns - col);
    return { id: index + 1, label: numberingMode === 'snake' ? snakeNumber : index + 1, row: row + 1, col: col + 1 };
  });
  const selected = cabinets.find((item) => item.id === selectedCabinet) ?? cabinets[0];

  const projectPayload = useMemo<ProjectState>(() => normalizeProject({
  ...
  const updateActiveScreen = (patch: Partial<ScreenDraft>) => setScreens((items) => items.map((item) => item.id === activeDraft.id ? { ...item, ...patch } : item));
  const updateScreen = (id: string, patch: Partial<ScreenDraft>) => setScreens((items) => items.map((item) => item.id === id ? { ...item, ...patch } : item));
  const canvasPointFromClient = (clientX: number, clientY: number) => { const rect = canvasRef.current?.getBoundingClientRect(); if (!rect) return { x: 260, y: 180 }; return { x: snap((clientX - rect.left - pan.x) / (zoom / 100), snapToGrid), y: snap((clientY - rect.top - pan.y) / (zoom / 100), snapToGrid) }; };

  const addScreen = (x?: number, y?: number) => { const nextNumber = screens.length + 1; const id = `screen-${Date.now()}`; const newScreen: ScreenDraft = { id, name: language === 'ru' ? `Экран ${nextNumber}` : `Screen ${nextNumber}`, cabinetId: cabinetPresets[0].id, columns: '6', rows: '3', x: x ?? 260 + nextNumber * 34, y: y ?? 180 + nextNumber * 28, visible: true, locked: false }; setScreens((items) => [...items, newScreen]); setActiveScreenId(id); setSelectedCabinet(1); setContextMenu(null); };
  const duplicateScreen = (screenId = activeScreenId) => { const source = screens.find((item) => item.id === screenId) ?? activeDraft; const id = `screen-${Date.now()}`; const copy = { ...source, id, name: language === 'ru' ? `${source.name} копия` : `${source.name} copy`, x: snap(source.x + 40, snapToGrid), y: snap(source.y + 40, snapToGrid), locked: false, visible: true }; setScreens((items) => [...items, copy]); setActiveScreenId(id); setSelectedCabinet(1); setContextMenu(null); };
  const deleteScreen = (screenId = activeScreenId) => { if (screens.length <= 1) return; const remaining = screens.filter((item) => item.id !== screenId); setScreens(remaining); setActiveScreenId(remaining[0].id); setSelectedCabinet(1); setContextMenu(null); };
  const fitWorkspace = () => { setZoom(90); setPan({ x: 0, y: 0 }); setContextMenu(null); };
  const handleWheel = (event: WheelEvent<HTMLDivElement>) => { event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(); const oldZoom = zoom / 100; const nextZoomValue = Math.min(180, Math.max(35, zoom + (event.deltaY > 0 ? -6 : 6))); const nextZoom = nextZoomValue / 100; const cursorX = event.clientX - rect.left; const cursorY = event.clientY - rect.top; const worldX = (cursorX - pan.x) / oldZoom; const worldY = (cursorY - pan.y) / oldZoom; setZoom(nextZoomValue); setPan({ x: cursorX - worldX * nextZoom, y: cursorY - worldY * nextZoom }); };

  const startScreenDrag = (event: PointerEvent<HTMLDivElement>, draft: ScreenDraft) => { const target = event.target as HTMLElement; if (target.closest('button') || draft.locked) return; event.currentTarget.setPointerCapture(event.pointerId); dragRef.current = { id: draft.id, startX: event.clientX, startY: event.clientY, originalX: draft.x, originalY: draft.y }; setDraggingScreenId(draft.id); setActiveScreenId(draft.id); setSelectedCabinet(1); };
  const moveScreenDrag = (event: PointerEvent<HTMLDivElement>) => { if (!dragRef.current) return; const drag = dragRef.current; let deltaX = (event.clientX - drag.startX) / (zoom / 100); let deltaY = (event.clientY - drag.startY) / (zoom / 100); if (event.shiftKey) { if (!drag.axis) drag.axis = Math.abs(deltaX) > Math.abs(deltaY) ? 'x' : 'y'; if (drag.axis === 'x') deltaY = 0; if (drag.axis === 'y') deltaX = 0; } else drag.axis = undefined; setScreens((items) => items.map((item) => item.id === drag.id ? { ...item, x: snap(drag.originalX + deltaX, snapToGrid), y: snap(drag.originalY + deltaY, snapToGrid) } : item)); };
  const endScreenDrag = () => { dragRef.current = null; setDraggingScreenId(null); };
  const startPan = (event: PointerEvent<HTMLDivElement>) => { if (event.button !== 1 && !event.altKey) return; event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); panRef.current = { startX: event.clientX, startY: event.clientY, originalX: pan.x, originalY: pan.y }; };
  const movePan = (event: PointerEvent<HTMLDivElement>) => { if (!panRef.current) return; setPan({ x: panRef.current.originalX + event.clientX - panRef.current.startX, y: panRef.current.originalY + event.clientY - panRef.current.startY }); };
  const endPan = () => { panRef.current = null; };
  const showStageContext = (event: ReactMouseEvent<HTMLDivElement>) => { event.preventDefault(); const point = canvasPointFromClient(event.clientX, event.clientY); setContextMenu({ x: event.clientX, y: event.clientY, target: 'stage', canvasX: point.x, canvasY: point.y }); };
  const showScreenContext = (event: ReactMouseEvent<HTMLDivElement>, screenId: string) => { event.preventDefault(); event.stopPropagation(); setActiveScreenId(screenId); setContextMenu({ x: event.clientX, y: event.clientY, target: 'screen', screenId }); };

  if (!isLoggedIn) return <main className="login-shell"><section className="login-card"><div className="login-mark">S</div><h1>{t.loginTitle}</h1><p>{t.loginSubtitle}</p><button className="tool-button primary" onClick={login}>{t.enterAsAdmin}</button><div className="lang login-lang"><button className={language === 'ru' ? 'active' : ''} onClick={() => setLanguage('ru')}>RU</button><button className={language === 'en' ? 'active' : ''} onClick={() => setLanguage('en')}>EN</button></div></section></main>;
  ...
  return (
    <main className="app-shell" onClick={() => setContextMenu(null)}>
      ...
      <section className="workspace">
        ...
        <aside className="inspector screen-builder"><h2>{screen.name}</h2><p className="inspector-note">{t.activeScreenNote}</p><div className="inspector-tabs five-tabs"><button className={inspectorTab === 'screen' ? 'active' : ''} onClick={() => setInspectorTab('screen')}>{t.screen}</button><button className={inspectorTab === 'cabinet' ? 'active' : ''} onClick={() => setInspectorTab('cabinet')}>{t.cabinet}</button><button className={inspectorTab === 'calc' ? 'active' : ''} onClick={() => setInspectorTab('calc')}>{t.calc}</button><button className={inspectorTab === 'canvas' ? 'active' : ''} onClick={() => setInspectorTab('canvas')}>{t.canvas}</button><button className={inspectorTab === 'project' ? 'active' : ''} onClick={() => setInspectorTab('project')}>{t.project}</button></div>{inspectorTab === 'screen' && <div className="tab-panel"><label className="field"><span>{t.screenName}</span><input value={activeDraft.name} onChange={(event) => updateActiveScreen({ name: event.target.value })} /></label><label className="field"><span>{t.cabinetType}</span><select value={activeDraft.cabinetId} onChange={(event) => updateActiveScreen({ cabinetId: event.target.value })}>{cabinetPresets.map((preset) => <option value={preset.id} key={preset.id}>{preset.name}</option>)}</select></label><div className="section-title">{t.cabinetGrid}</div><div className="field-row"><label className="field"><span>{t.width}</span><input value={activeDraft.columns} inputMode="numeric" onChange={(event) => updateActiveScreen({ columns: event.target.value })} /></label><label className="field"><span>{t.height}</span><input value={activeDraft.rows} inputMode="numeric" onChange={(event) => updateActiveScreen({ rows: event.target.value })} /></label></div><div className="segmented"><button className={activeDraft.visible !== false ? 'active' : ''} onClick={() => updateActiveScreen({ visible: true })}>{t.visible}</button><button className={activeDraft.locked ? 'active' : ''} onClick={() => updateActiveScreen({ locked: !activeDraft.locked })}>{t.locked}</button></div><div className="section-title">{t.numbering}</div><div className="segmented"><button className={numberingMode === 'rows' ? 'active' : ''} onClick={() => setNumberingMode('rows')}>{t.rows}</button><button className={numberingMode === 'snake' ? 'active' : ''} onClick={() => setNumberingMode('snake')}>{t.snake}</button></div><button className="danger-button" onClick={() => deleteScreen()} disabled={screens.length <= 1}>{t.deleteScreen}</button></div>}{inspectorTab === 'cabinet' && <div className="tab-panel"><div className="section-title">{t.selectedCabinetTitle}</div><div className="prop"><span>ID</span><strong>#{selected?.label ?? 1}</strong></div><div className="prop"><span>{t.position}</span><strong>{t.row} {selected?.row ?? 1}, {t.column} {selected?.col ?? 1}</strong></div><div className="prop"><span>{t.size}</span><strong>{cabinet.widthMm} × {cabinet.heightMm} mm</strong></div><div className="prop"><span>{t.resolution}</span><strong>{cabinet.pixelsX} × {cabinet.pixelsY} px</strong></div><div className="section-title">{t.model}</div><div className="prop"><span>{t.name}</span><strong>{cabinet.name}</strong></div><div className="prop"><span>{t.manufacturer}</span><strong>{cabinet.manufacturer}</strong></div><div className="prop"><span>{t.weight}</span><strong>{cabinet.weightKg} kg</strong></div><div className="prop"><span>{t.averagePower}</span><strong>{cabinet.avgPowerW} W</strong></div><div className="prop"><span>{t.maxPower}</span><strong>{cabinet.maxPowerW} W</strong></div><div className="prop"><span>{t.config}</span><strong>{cabinet.configFile}</strong></div></div>}{inspectorTab === 'calc' && <div className="tab-panel"><div className="section-title">{t.activeScreen}</div><div className="prop"><span>{t.cabinets}</span><strong>{screen.calculated.cabinets}</strong></div><div className="prop"><span>{t.physicalSize}</span><strong>{screen.calculated.widthM} × {screen.calculated.heightM} m</strong></div><div className="prop"><span>{t.resolution}</span><strong>{screen.calculated.resolutionX} × {screen.calculated.resolutionY}</strong></div><div className="prop"><span>{t.area}</span><strong>{screen.calculated.areaM2} m²</strong></div><div className="prop"><span>{t.weight}</span><strong>{screen.calculated.weightKg} kg</strong></div><div className="prop"><span>{t.averagePower}</span><strong>{screen.calculated.avgPowerKw} kW</strong></div><div className="prop"><span>{t.maxPower}</span><strong>{screen.calculated.maxPowerKw} kW</strong></div><div className="section-title">{t.wholeProject}</div><div className="prop"><span>{t.screens}</span><strong>{screens.length}</strong></div><div className="prop"><span>{t.cabinets}</span><strong>{projectTotals.cabinets}</strong></div><div className="prop"><span>{t.area}</span><strong>{projectTotals.areaM2.toFixed(2)} m²</strong></div><div className="prop"><span>{t.weight}</span><strong>{projectTotals.weightKg.toFixed(1)} kg</strong></div><div className="prop"><span>{t.power}</span><strong>{projectTotals.avgPowerKw.toFixed(2)} / {projectTotals.maxPowerKw.toFixed(2)} kW</strong></div></div>}{inspectorTab === 'canvas' && <div className="tab-panel"><div className="section-title">CAD</div><div className="segmented"><button className={snapToGrid ? 'active' : ''} onClick={() => setSnapToGrid(true)}>{t.snapToGrid}</button><button className={!snapToGrid ? 'active' : ''} onClick={() => setSnapToGrid(false)}>Free</button></div><div className="segmented canvas-switch"><button className={showRulers ? 'active' : ''} onClick={() => setShowRulers((value) => !value)}>{t.rulers}</button><button className={showGuides ? 'active' : ''} onClick={() => setShowGuides((value) => !value)}>{t.guides}</button></div><div className="prop"><span>Zoom</span><strong>{zoom}%</strong></div><div className="prop"><span>Pan</span><strong>{Math.round(pan.x)}, {Math.round(pan.y)}</strong></div><p className="inspector-note canvas-hint">{t.panHint}</p></div>}{inspectorTab === 'project' && <div className="tab-panel"><div className="section-title">{t.projectInfo}</div><label className="field"><span>{t.name}</span><input value={projectName} onChange={(event) => setProjectName(event.target.value)} /></label><label className="field"><span>{t.description}</span><input value={description} onChange={(event) => setDescription(event.target.value)} /></label><label className="field"><span>{t.author}</span><input value={author} onChange={(event) => setAuthor(event.target.value)} /></label><div className="prop"><span>{t.version}</span><strong>v{version}</strong></div><div className="prop"><span>{t.created}</span><strong>{new Date(createdAt).toLocaleDateString(language === 'ru' ? 'ru-RU' : 'en-US')}</strong></div><button className="create-screen-button" onClick={() => saveProjectNow(projectPayload, true)}>{t.saveProject}</button></div>}</aside>
      </section>
      <footer className="statusbar"><div className="status-item"><span>{t.project}</span><strong>{projectName}</strong></div><div className="status-item"><span>{t.version}</span><strong>v{version}</strong></div><div className="status-item"><span>{t.cabinets}</span><strong>{projectTotals.cabinets}</strong></div><div className="status-item"><span>{t.area}</span><strong>{projectTotals.areaM2.toFixed(2)} m²</strong></div><div className="status-item"><span>{t.power}</span><strong>{projectTotals.avgPowerKw.toFixed(2)} / {projectTotals.maxPowerKw.toFixed(2)} kW</strong></div></footer>
    </main>
  );
}
